        ...
        <div className="relative">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-8 sm:gap-10 relative z-10">
            {steps.map((step, idx) => (
              <div
                key={idx}
                className={`bg-white p-6 sm:p-8 rounded-2xl shadow-sm border border-gray-100 text-center md:text-left transition-all duration-300 hover:-translate-y-2 hover:shadow-xl`}
              >
                <div
                  className={`w-16 h-16 sm:w-20 sm:h-20 rounded-full flex items-center justify-center font-bold text-xl sm:text-2xl mb-6 sm:mb-8 mx-auto md:mx-0 shadow-sm ring-4 sm:ring-8 ring-white ${
                    step.active
                      ? 'bg-primary text-white'
                      : 'bg-gray-50 text-secondary border-2 border-gray-200'
                  }`}
                >
                  {step.num}
                </div>
